...
def run_knn():
    ...
    #####################################################################
    for k in range(50):
        train_labels = knn(k, train_inputs, train_targets, valid_inputs)
        val_labels = knn(k, valid_inputs, valid_targets, valid_inputs)
        x = np.sum(val_labels) / 50
        plt.plot(k,x, "ro" )
    test_labels = knn(k, test_inputs, test_targets, valid_inputs)
    x = np.sum(test_labels) / 50
    print("Test Score: {}".format(x))
